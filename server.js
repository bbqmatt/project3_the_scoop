...
//A function to create a comments
//Receives comment information from comment property of request body
//Creates new comment and adds it to database, returns a 201 response with
//    comment on comment property of response body
//If body isn't supplied, user with supplied username doesn't exist, or
//   article with supplied article ID doesn't exist, returns a 400 response

function createComment(url, request) {
  const requestComment = request.body && request.body.comment;
  const response = {};

  //check that body is supplied with an id and correct username.
  ...
  if (requestComment && requestComment.body
    ...
    //console.log(database.users[comment.username].commentIds); //debug
...
//update a comment
//Receives comment ID from URL parameter and updated comment from comment
...
function updateComment(url, request){
  ...
  //retrieve the comment from the database using the id on the URL
...
// If no ID is supplied or comment with supplied ID doesn't exist,
...
function deleteComment(url, request){
  ...
  const savedComment = database.comments[id];
  //console.log(savedComment); //debug

  if (savedComment){

      //code to delete the comment
      //delete the comment
      //console.log(database.comments[savedComment.id]); //debug
      database.comments[savedComment.id] = null;
      //console.log(database.comments[savedComment.id]); //debug

      const userCommentIds = database.users[savedComment.username].commentIds;
      //console.log(userCommentIds); //debug
      //console.log(userCommentIds.indexOf(id)); //debug
      userCommentIds.splice(userCommentIds.indexOf(id), 1);

      //delete the comment ID from the articles comment ID
      const articleCommentIds = database.articles[savedComment.articleId].commentIds;
      //console.log(articleCommentIds);
...
/* Receives comment ID from URL parameter and username from username property
    of request body
Adds supplied username to upvotedBy of corresponding comment if user
    hasn't already upvoted the comment, removes username from downvotedBy
    if that user had previously downvoted the comment, returns 200 response
    with comment on comment property of response body
If no ID is supplied, comment with supplied ID doesn't exist, or user with
    supplied username doesn't exist, returns 400 response */
...
/* Receives comment ID from URL parameter and username from username property
    of request body
Adds supplied username to downvotedBy of corresponding comment if user hasn't
    already downvoted the comment, remove username from upvotedBy if that
    user had previously upvoted the comment, returns 200 response with comment
    on comment property of response body
If no ID is supplied, comment with supplied ID doesn't exist, or user with
    supplied username doesn't exist, returns 400 response */
